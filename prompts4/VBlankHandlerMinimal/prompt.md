You are decompiling an assembly function called `VBlankHandlerMinimal` in ARMv4T from a Game Boy Advance game.
...
Decompile the following target assembly function from `asm/nonmatchings/math/VBlankHandlerMinimal.s` into clean, readable C code that compiles to an assembly matching EXACTLY the original one.

```asm
	non_word_aligned_thumb_func_start VBlankHandlerMinimal
VBlankHandlerMinimal: @ 08000AB2
	push {lr}
	bl FUN_0804f8e8
	bl SoundInit
	ldr r1, [pc, #0x008] @ =0x03007FF8
	movs r0, #0x01
	strh r0, [r1, #0x00]
	pop {r0}
	bx r0
