You are decompiling an assembly function called `ply_pan` in ARMv4T from a Game Boy Advance game.
...
Decompile the following target assembly function from `asm/nonmatchings/m4a/ply_pan.s` into clean, readable C code that compiles to an assembly matching EXACTLY the original one.

```asm
	non_word_aligned_thumb_func_start ply_pan
ply_pan: @ 080513CE
	ldr r0, [r1, #0x40]
	ldrb r0, [r0, #0x00]
	adds r2, r1, #0x0
