You are decompiling an assembly function called `StrCmp` in ARMv4T from a Game Boy Advance game.
...
Decompile the following target assembly function from `asm/nonmatchings/system/StrCmp.s` into clean, readable C code that compiles to an assembly matching EXACTLY the original one.
